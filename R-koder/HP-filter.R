...
BNP_t<- ts(log(data$bnp), start=1971, freq=4)
Kredit<-ts(log(data$kredit), start=1971, freq=4)
Boligpriser <- ts(log(data$huspriser2), start=1971, freq=4)

#### HP-filter ####
BNP.hp <- hpfilter(BNP_t, freq = 400000) #32.000 #100.000
plot(BNP.hp)

Kredit.hp <- hpfilter(Kredit, freq = 400000) #32.000 #100.000
plot(Kredit.hp)

Boligpriser.hp <- hpfilter(Boligpriser, freq = 400000) #32.000 #100.000
plot(Boligpriser.hp)
...
#### GGPLOT med begge y akser ####
 ...
 p <- ggplot() + 
  ...
  labs(title ="", x = "Årstal", y = "Cykliske udsving (%)")  + labs(color="") +
...
#ggsave("Stresstesttest", t_1, width = 30,height = 12,units = "cm",path ="~/Desktop")
